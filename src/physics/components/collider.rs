...
impl Collider {
    ...
    pub fn broad_phase_check(&self, rel_velocity: Vec2) -> Collider {
        let mut broadphasebox = Collider::ZERO;
        broadphasebox.x = if rel_velocity.x > 0.0 {
            self.x
        } else {
            self.x + rel_velocity.x
        };
        broadphasebox.y = if rel_velocity.y > 0.0 {
            self.y
        } else {
            self.y + rel_velocity.y
        };
        broadphasebox.w = if rel_velocity.x > 0.0 {
            rel_velocity.x + self.w
        } else {
            self.w - rel_velocity.x
        };
        broadphasebox.h = if rel_velocity.y > 0.0 {
            rel_velocity.y + self.h
        } else {
            self.h - rel_velocity.y
        };
