...
use crate::physics::{
    Dynamic, Static,
    components::{collider::Collider, velocity::Velocity},
...
fn constraint_system(mut query: Query<(&mut Transform, &Collider, &Constraint)>) {
...
fn main() {
    App::new()
        ...
        .add_systems(FixedPostUpdate, constraint_system)
        .run();
}
...
fn paddle_collision_observer(
    ...
    let Ok(paddle_collider) = colliders.get(event.entity) else {
        return;
    };
...
fn create_paddle(commands: &mut Commands, speed: f32, side: Side) {
    ...
    commands
        .spawn((
            ...
            Static,
            match side {
                Side::Left => Constraint::new(Vec2::new(-360.0, -243.0), Vec2::new(-340.0, 243.0)),
                Side::Right => Constraint::new(Vec2::new(340.0, -243.0), Vec2::new(380.0, 243.0)),
            },
        ))
...
fn setup(mut commands: Commands) {
    ...
    //spawn walls
    commands.spawn((
        Sprite::from_color(Color::WHITE, Vec2::new(800.0, 10.0)),
